...
$tpl = erLhcoreClassTemplate::getInstance('lhfbmessenger/channels.tpl.php');
...
$pages->items_total = erLhcoreClassModelFBChannel::getCount($filterParams['filter']);
$pages->translationContext = 'chat/activechats';
$pages->serverURL = erLhcoreClassDesign::baseurl('fbmessenger/channels').$append;
$pages->paginate();
$tpl->set('pages',$pages);

if ($pages->items_total > 0) {
    $items = erLhcoreClassModelFBChannel::getList(array_merge(array('limit' => $pages->items_per_page, 'offset' => $pages->low),$filterParams['filter']));
    $tpl->set('items',$items);
}

$filterParams['input_form']->form_action = erLhcoreClassDesign::baseurl('fbmessenger/channels');
...
$Result['path'] = array(
    ...
    array('title' => erTranslationClassLhTranslation::getInstance()->getTranslation('module/fbmessenger','Segment Tags'))
);
